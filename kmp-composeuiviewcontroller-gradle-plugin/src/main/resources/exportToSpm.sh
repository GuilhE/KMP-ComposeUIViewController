...
# Builds the linkerSettings unsafeFlags array for the Representables target.
# ObjC Export only: explicitly links the .framework so the linker never relies on Swift auto-linking,
# which is fragile in SPM static-library contexts and causes intermittent "Undefined symbols" errors.
# Swift Export takes priority — no linker settings are needed when dd-interfaces are present.
build_linker_flags() {
  if [ -d "$KMP_INTERFACES_ABS" ]; then
    return 0
  fi
  if [ -n "$OBJC_PLATFORM_DIR" ] && [ -d "$KMP_OBJC_FRAMEWORK_PARENT_ABS" ] && [ -n "$OBJC_FRAMEWORK_NAME" ]; then
    echo '"-F", "'"$KMP_OBJC_FRAMEWORK_RELPATH"'", "-framework", "'"$OBJC_FRAMEWORK_NAME"'"'
  fi
}
