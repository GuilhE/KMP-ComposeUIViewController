...
|--------------------------------------------------------------------------------------------------|:------------------------------------------------------:|:---------------------------------------------:|:------------------------------------------------------------------------------------:|:------:|
| 2.4.20-1.12.0                                                                                    |                         2.4.20                         |                    2.3.11                     |                                        1.12.0                                        | 26.6.0 |
...
- `exportFolderName` name of the folder inside the iOS project (`iosAppFolderName`) that holds the generated files when `autoExport` is `true`. By default, SPM export, this is the local Swift Package folder — containing `Package.swift` and `Sources/{exportFolderName}/` — created by `createRepresentablesPackage` and kept in sync on every build. When `legacyMode` is `true`, the generated `UIViewControllerRepresentable` files are copied directly into this folder instead;
- `iosDeploymentTarget` minimum iOS version for the generated `Package.swift` (e.g. `"16"`). Not used when `legacyMode` is `true`;
- `swiftToolsVersion` Swift tools version for the generated `Package.swift` (e.g. `"5.9"`, `"6.0"`). Not used when `legacyMode` is `true`.
- `autoExport` enables auto export generated files to Xcode project on every build. By default, SPM export, this keeps the local Swift Package at `iosAppFolderName/exportFolderName/` in sync with the KSP output. When `legacyMode` is `true`, it instead copies the files directly and updates `project.pbxproj`. If set to `false`, neither happens, and you will find the generated files under `/build/generated/ksp/`. Warning: avoid deleting `iosAppFolderName/exportFolderName` without first using Xcode to `Remove references`, when using legacy mode;
- `legacyMode` when `true`, falls back to the previous `xcodeproj`-gem-based export instead of the default local SPM package at `[iosAppFolderName]/[exportFolderName]/`. Kept for projects that can't yet use SPM — new projects should not need this. See [Legacy mode](#legacy-mode);
...
### SPM export
This is the default export mode (see [Legacy mode](#legacy-mode) for the previous `xcodeproj`-gem-based approach). It works with both **Swift Export** and **ObjC Export**, generating a local Swift Package with the generated `UIViewControllerRepresentable` files — nothing to enable, just configure the usual parameters:
...
Run this task once, before your first Xcode build. It creates the local Swift Package stub and automatically adds the package reference to your Xcode project — no manual Xcode changes needed:
...
### Legacy mode
Falls back to the previous `xcodeproj`-gem-based export: it manipulates `project.pbxproj` directly on every build instead of using a local SPM package. Kept for projects that can't yet use SPM — new projects should not need this.
...
No one-time setup step is required — `copyFilesToXcode` manages the `project.pbxproj` references automatically on every build.
...
```
> [!IMPORTANT]
> When using [legacy mode](#legacy-mode), always delete `iosApp/Representables` through Xcode.

## Sample
For working samples check [sample-objc-export](sample-objc-export), [sample-objc-export-legacy](sample-objc-export-legacy), [sample-swift-export](sample-swift-export) or [sample-swift-export-legacy](sample-swift-export-legacy). 
