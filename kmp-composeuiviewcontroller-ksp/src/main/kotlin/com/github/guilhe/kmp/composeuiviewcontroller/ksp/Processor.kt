...
internal class Processor(
	...
	private fun createKotlinFileWithoutState(
		...
		makeParameters: List<KSValueParameter>,
		parameters: List<KSValueParameter>,
		opaqueConfiguration: Boolean
	): String {
		val allImports = (imports + listOf(
			"androidx.compose.ui.window.ComposeUIViewController",
			"platform.UIKit.UIViewController"
		)).distinct().sorted()
		val importsParsed = allImports.joinToString("\n") { "import $it" }
		val code = """
            ...
            object ${composable.name()}UIViewController {
                fun make(${makeParameters.joinToStringDeclaration()}): UIViewController {
                    return ComposeUIViewController(configure = { opaque = $opaqueConfiguration }) {
	...
	private fun createSwiftFileWithoutState(
		...
		val letParameters = makeParameters.joinToString("\n") {
			...
			"public let ${it.name()}: $finalType"
		}
		val publicInit = if (makeParameters.isNotEmpty()) {
			val initParams = makeParameters.joinToString(", ") {
				val type = it.resolveType(toSwift = true, withSwiftExport = swiftExportEnabled)
				val finalType = externalParameters[type] ?: type
				"${it.name()}: ${finalType.withEscapingIfNeeded()}"
			}
			val initBody = makeParameters.joinToString("\n") { "self.${it.name()} = ${it.name()}" }
        ...
        """.trimIndent()
		val indentedCode = indentParameters(indentParameters(indentParameters(code, frameworks), letParameters), publicInit)
		val updatedCode = indentedCode.plus("\n")
		codeGenerator
			.createNewFile(
				dependencies = Dependencies(true),
				packageName = "",
	...
	private fun createSwiftFileWithState(
		...
		val initParams = buildList {
			add("$stateParameterName: Binding<$finalStateType>")
			makeParameters.forEach {
				val type = it.resolveType(toSwift = true, withSwiftExport = swiftExportEnabled)
				val finalType = externalParameters[type] ?: type
				add("${it.name()}: ${finalType.withEscapingIfNeeded()}")
			}
		}.joinToString(", ")
		val initBody = buildList {
			add("self._$stateParameterName = $stateParameterName")
			makeParameters.forEach { add("self.${it.name()} = ${it.name()}") }
		}.joinToString("\n")
        ...
        """.trimIndent()
		val updatedCode = indentParameters(indentParameters(indentParameters(code, frameworks), letParameters), publicInit).plus("\n")
		codeGenerator
			.createNewFile(
				dependencies = Dependencies(true),
				packageName = "",
	...
	// Closures stored as properties escape the init scope, so init parameters of function type
	// require @escaping. Optional closures (ending with ?) are implicitly escaping — no annotation needed.
	private fun String.withEscapingIfNeeded(): String =
		if (contains("->") && !endsWith("?")) "@escaping $this" else this
