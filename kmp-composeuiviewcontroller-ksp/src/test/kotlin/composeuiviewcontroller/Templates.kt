...
object Templates {
	...
	object ExpectedOutputs {
		private fun withEscapingIfNeeded(type: String): String =
			if (type.contains("->") && !type.endsWith("?")) "@escaping $type" else type
		...
		fun swiftRepresentableWithoutState(
			...
			val letProps = params.joinToString("\n    ") { "public let ${it.first}: ${it.second}" }
			val initParams = params.joinToString(", ") { (name, type) -> "$name: ${withEscapingIfNeeded(type)}" }
			val initBody = params.joinToString("\n    ") { "self.${it.first} = ${it.first}" }
			val publicInit = "public init($initParams) {\n    $initBody\n    }"
			val makeParams = params.joinToString(", ") { "${it.first}: ${it.first}" }
			return """
// This file is auto-generated by KSP. Do not edit manually.
import $framework
import SwiftUI
...
public struct ${functionName}Representable: UIViewControllerRepresentable {
    ...
    public func makeUIViewController(context _: Context) -> UIViewController {
        ${functionName}UIViewController${if (isSharedInstance) ".shared" else "()"}.make($makeParams)
    }
...
""".trimIndent()
		...
		fun swiftRepresentableWithState(
            ...
            public struct ${functionName}Representable: UIViewControllerRepresentable {
                @Binding public var state: $stateType
		...
		fun swiftRepresentableWithObjCTypes(
            ...
            public struct ${functionName}Representable: UIViewControllerRepresentable {
                @Binding public var state: ViewState
                public let callBackA: () -> Void
                public let callBackB: (Array<Dictionary<String, Array<KotlinInt>>>) -> Array<String>
                public let callBackS: (Set<KotlinInt>) -> Void
                public let callBackC: (NSMutableArray) -> Void
                public let callBackD: (Dictionary<String, String>) -> Void
                public let callBackE: (NSMutableDictionary) -> Void
                public let callBackF: (KotlinByte) -> Void
                public let callBackG: (KotlinUByte) -> Void
                public let callBackH: (KotlinShort) -> Void
                public let callBackI: (KotlinUShort) -> Void
                public let callBackJ: (KotlinInt) -> Void
                public let callBackK: (KotlinUInt) -> Void
                public let callBackL: (KotlinLong) -> Void
                public let callBackM: (KotlinULong) -> Void
                ...
                self.callBackC = callBackC
                self.callBackD = callBackD
                self.callBackE = callBackE
                self.callBackF = callBackF
                self.callBackG = callBackG
                self.callBackH = callBackH
                self.callBackI = callBackI
                self.callBackJ = callBackJ
                self.callBackK = callBackK
                self.callBackL = callBackL
                self.callBackM = callBackM
                self.callBackN = callBackN
                self.callBackO = callBackO
                self.callBackP = callBackP
                }
		...
		fun swiftRepresentableWithSwiftExportTypes(
            ...
            public struct ${functionName}Representable: UIViewControllerRepresentable {
                @Binding public var state: ViewState
                public let callBackA: () -> Void
                public let callBackB: (Array<Dictionary<String, Array<Int32>>>) -> Array<String>
                public let callBackS: (Set<Int32>) -> Void
                public let callBackC: (Array<String>) -> Void
                public let callBackD: (Dictionary<String, String>) -> Void
                public let callBackE: (Dictionary<String, String>) -> Void
                public let callBackF: (Int8) -> Void
                public let callBackG: (UInt8) -> Void
                public let callBackH: (Int16) -> Void
                public let callBackI: (UInt16) -> Void
                public let callBackJ: (Int32) -> Void
                public let callBackK: (UInt32) -> Void
                public let callBackL: (Int64) -> Void
                public let callBackM: (UInt64) -> Void
